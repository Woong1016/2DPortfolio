...
// 만들어진 이펙트 매니저를 씬에서 활용해보기

// 1. 닌자와 적(NPC) 혹은 때릴 수 있는 물건(충돌체가 있는 물체) 배치 후
//    닌자가 상대를 맞추면 그 자리에 이펙트가 재생되도록 해봅시다

// 2. 점 충돌 예습을 겸해서, 씬에 사각형 충돌체를 (가능하면 그림도 있으면 더 좋습니다) 띄운 후,
//    해당 충돌 영역을 클릭하면 클릭한 자리에 이펙트가 재생되도록 해봅시다.


// (옵션) 제공된 리소스 말고도 다른 이펙트가 있는 스프라이트가 있을까요? 찾아봅시다!
